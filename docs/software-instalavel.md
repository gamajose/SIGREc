...
Ele usa Java 21, aponta para o banco da VM `192.168.28.128:5432/segrec` e abre `http://localhost:8080` automaticamente.
...
```powershell
jpackage --type exe --name SIGREc --input target --main-jar tfd-apac-0.1.0.jar --main-class org.springframework.boot.loader.launch.JarLauncher --java-options "-DDB_HOST=192.168.28.128" --java-options "-DDB_NAME=segrec" --java-options "-DDB_USER=jose" --java-options "-DDB_PASSWORD=Joseluiz1"
```
