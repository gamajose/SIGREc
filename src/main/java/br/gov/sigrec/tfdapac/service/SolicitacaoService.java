...
public class SolicitacaoService {
    ...
    public List<Map<String, Object>> fila(String status, String tipo, String termo) {
        ...
        return jdbcTemplate.queryForList("""
                select s.*,
                       ...
                       p.cns paciente_cns,
                       u.nome unidade_nome,
                       ua.nome unidade_autorizadora_nome,
    ...
    public List<Map<String, Object>> filaDoUsuario(String status, String tipo, String termo,
            Long usuarioId, Long unidadeId, boolean admin) {
        if (admin) {
    ...
    public Map<String, Object> detalhe(Long id) {
        return jdbcTemplate.queryForMap("""
                select s.*,
                       ...
                       p.cpf paciente_cpf,
                       p.rg paciente_rg,
                       p.data_nascimento paciente_nascimento,
                       p.sexo paciente_sexo,
                       p.nome_mae paciente_mae,
                       p.endereco paciente_endereco,
                       p.municipio paciente_municipio,
                       p.uf paciente_uf,
                       p.cep paciente_cep,
                       p.telefone paciente_telefone,
                       p.email paciente_email,
                       u.nome unidade_nome,
                       u.cnes unidade_cnes,
                       u.municipio unidade_municipio,
                       u.uf unidade_uf,
                       ua.nome unidade_autorizadora_nome,
                       ua.cnes unidade_autorizadora_cnes,
                       ps.nome profissional_solicitante_nome,
                       ps.cpf_cns profissional_solicitante_doc,
                       ps.conselho profissional_solicitante_conselho,
                       ps.registro_conselho profissional_solicitante_registro,
    ...
    public Map<String, Object> tfd(Long id) {
        return jdbcTemplate.queryForMap(
                "select * from regulacao_tfd.solicitacao_tfd where solicitacao_id = ?",
                id
        );
    }
    ...
    public Map<String, Object> apac(Long id) {
        return jdbcTemplate.queryForMap(
                "select * from regulacao_tfd.solicitacao_apac where solicitacao_id = ?",
                id
        );
    }
    ...
    public Long criar(Map<String, String> form, Long usuarioId) {
        ...
        if ("TFD".equals(tipo)) {
            jdbcTemplate.update("""
                    ...
                    form.get("procedimento_exame_indicado"),
                    form.get("sinais_sintomas"),
                    booleanValue(form, "necessita_acompanhante"),
                    form.get("acompanhante_nome"),
                    date(form.get("acompanhante_data_nascimento"), "Data de nascimento do acompanhante"),
                    form.get("acompanhante_cpf"),
                    form.get("acompanhante_telefone"),
                    form.get("destino"),
                    form.get("justificativa_medica")
            );
        } else {
            jdbcTemplate.update("""
                    ...
                    form.get("ibge"),
                    intValue(form, "quantidade", 1),
                    form.get("causas_associadas"),
                    form.get("observacoes"),
                    date(form.get("data_solicitacao"), "Data da solicitação"),
                    form.get("numero_apac"),
                    date(form.get("validade_inicio"), "Validade inicial"),
                    date(form.get("validade_fim"), "Validade final"),
                    form.get("estabelecimento_executante"),
                    form.get("cnes_executante")
            );
        }

        historico(id, usuarioId, null, "ENVIADA", "CRIACAO", "Solicitação enviada para regulação");
    ...
    public void atualizar(Long id, Map<String, String> form, Long usuarioId) {
        String status = jdbcTemplate.queryForObject(
                "select status from regulacao_tfd.solicitacoes where id = ?",
                String.class,
                id
        );

        if (!"RASCUNHO".equals(status) && !"ENVIADA".equals(status) && !"DEVOLVIDA_CORRECAO".equals(status)) {
            throw new IllegalStateException("Somente requisições em rascunho, enviadas ou devolvidas podem ser editadas.");
        }
        ...
        String tipo = jdbcTemplate.queryForObject(
                "select tipo_solicitacao from regulacao_tfd.solicitacoes where id = ?",
                String.class,
                id
        );
        ...
        historico(id, usuarioId, status, status, "EDICAO", form.getOrDefault("observacao", "Requisição atualizada"));
    }

    @Transactional
    public void mudarStatus(Long id, String novoStatus, String observacao, Long usuarioId, Map<String, String> form) {
        String anterior = jdbcTemplate.queryForObject(
                "select status from regulacao_tfd.solicitacoes where id = ?",
                String.class,
                id
        );
        ...
        jdbcTemplate.update("""
                ...
                set status = ?,
                    ...
                    data_autorizacao = case
                        when ? = 'AUTORIZADA' then current_timestamp
                        else data_autorizacao
                    end,
                    usuario_ultima_alteracao = ?
                where id = ?
                """,
                ...
                novoStatus,
                observacao,
                nullableLong(form, "profissional_autorizador_id"),
                novoStatus,
                novoStatus,
                usuarioId,
                id
        ...
        notificationService.notificarUsuario(
                usuarioCriacao,
                tituloNotificacao(novoStatus),
                mensagemNotificacao(novoStatus),
                "/solicitacoes/" + id
        );
    }
    ...
    private String tituloNotificacao(String status) {
        return switch (status) {
            ...
            case "AGUARDANDO_DOCUMENTOS" -> "Solicitação pendente de documentação";
            default -> "Solicitação atualizada";
        };
    }

    private String mensagemNotificacao(String status) {
        return switch (status) {
            ...
            case "AGUARDANDO_DOCUMENTOS" -> "Sua solicitação possui pendência de documentação.";
            default -> "O status da sua solicitação foi atualizado para " + status + ".";
        };
    }
    ...
    private String required(Map<String, String> form, String key) {
        ...
        if (!StringUtils.hasText(value)) {
            throw new IllegalArgumentException("Campo obrigatório: " + key);
        }
        return value;
    }
